...
- Visual, layout, responsividade e microinterações do novo frontend: `docs/design-reference/claude-design/` (referência aprovada, não aplicação, runtime ou fonte de arquitetura).
...
Graphify descreve relações encontradas no **código**. `docs/product-vault/`, `docs/design-reference/` e o código vendorizado (`apps/evolution-go/whatsmeow-lib/`) estão fora do grafo por configuração — ver `.graphifyignore`. Ele não substitui o product vault, não trata a referência visual como código e não transforma implementação legada em regra de produto.
