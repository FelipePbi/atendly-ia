...
export class MessageOrchestrator {
  ...
  private async handleFromMeMessage(message: ChannelInboundMessage): Promise<OrchestratorResult> {
    ...
    if (aiCommand?.type === "PAUSE_AI_FOR_CONTACT") {
      this.cancelBufferedMessages(message.customerPhone);
      this.logger.info(channelMessageLogContext(message), "Orchestrator received /ia_pause command");
    ...
    if (isTextMessage(message)) {
      this.cancelBufferedMessages(message.customerPhone);
      this.logger.info(channelMessageLogContext(message), "Orchestrator recording manual fromMe message");
    ...
    this.logger.info(channelMessageLogContext(message), "Orchestrator recorded manual owner activity");
    return { ok: true, action: "manual_activity_recorded" };
  }

  async flushBufferedMessagesForTesting(phone: string): Promise<OrchestratorResult | null> {
    return this.flushBufferedMessages(phone);
  }
  ...
  private async replyWithAssistant(args: {
    ...
    try {
      const botPausedBeforeAssistant = await this.handoff.isBotPaused(message.customerPhone);
      if (botPausedBeforeAssistant) {
        this.logger.info(channelMessageLogContext(message), "Orchestrator skipped assistant because bot is paused");
        return { ok: true, action: "paused_conversation" };
      }
      ...
      const botPausedBeforeSend = await this.handoff.isBotPaused(message.customerPhone);
      if (botPausedBeforeSend) {
        this.logger.info(channelMessageLogContext(message), "Orchestrator skipped send because bot was paused before delivery");
        return { ok: true, action: "paused_conversation" };
      }
  ...
  private async bufferTextMessage(message: ChannelInboundMessage & { kind: "text"; text: string }): Promise<OrchestratorResult> {
    ...
    this.logger.info(
      {
        ...
        scheduledProcessAt: getScheduledProcessAt(buffer, this.debounce).toISOString()
      },
  ...
  private scheduleBufferedProcessing(buffer: ConversationMessageBuffer): void {
    ...
    buffer.timer = setTimeout(() => {
      void this.flushBufferedMessages(buffer.phone).catch((error) => {
        this.logger.error({ phone: buffer.phone, err: toErrorMessage(error) }, "Orchestrator failed to flush buffered messages");
      });
    }, delayMs);
  }

  private cancelBufferedMessages(phone: string): void {
    const buffer = this.buffers.get(phone);
    if (!buffer) return;
    if (buffer.timer) clearTimeout(buffer.timer);
    this.buffers.delete(phone);
    this.logger.info({ phone }, "Orchestrator cancelled buffered messages");
  }

  private async flushBufferedMessages(phone: string): Promise<OrchestratorResult | null> {
    const buffer = this.buffers.get(phone);
    ...
    const text = buffer.messages.map((item) => item.text.trim()).filter(Boolean).join("\n");
    return this.replyWithAssistant({
      message: latest.channelMessage,
      text,
      bufferedMessageRecordIds: buffer.messages.map((item) => item.messageRecordId)
    });
  }
}
...
function hasBufferedAutomation(automation: AutomationPort | AssistantService): automation is AutomationPort &
  Required<Pick<AutomationPort, "recordInboundText" | "handleBufferedText">> {
  return typeof automation.recordInboundText === "function" && typeof automation.handleBufferedText === "function";
}

function buildDebounceConfig(overrides: MessageOrchestratorOptions["debounce"]): MessageDebounceConfig {
  if (overrides === false) {
    return {
      enabled: false,
      minSeconds: 0,
  ...
  const maxSeconds = Math.max(minSeconds, overrides?.maxSeconds ?? env.AI_DEBOUNCE_MAX_SECONDS);
  const maxWaitSeconds = Math.max(minSeconds, overrides?.maxWaitSeconds ?? env.AI_DEBOUNCE_MAX_WAIT_SECONDS);
...
function getScheduledProcessAt(buffer: ConversationMessageBuffer, config: MessageDebounceConfig): Date {
  const lastMessageAt = buffer.lastMessageAt.getTime();
  const maxRunAt = buffer.firstMessageAt.getTime() + config.maxWaitSeconds * 1000;
  const delayMs = getDebounceDelayMs(buffer, config);
  return new Date(Math.min(lastMessageAt + delayMs, maxRunAt));
}

function getDebounceDelayMs(buffer: ConversationMessageBuffer, config: MessageDebounceConfig): number {
...
function isUrgentFollowUp(text: string): boolean {
  ...
  return normalized.includes("???") || normalized === "alo" || normalized === "alô" || normalized === "ta ai?" || normalized === "tá aí?";
}
