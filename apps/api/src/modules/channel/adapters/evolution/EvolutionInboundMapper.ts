...
const messageEvents = new Set(["Message", "SendMessage", "MESSAGE", "SEND_MESSAGE"]);
...
export function mapEvolutionInbound(payload: unknown): ChannelInboundMessage | null {
  ...
  const instanceId = stringValue(payload.instanceId) || stringValue(payload.instance);
...
export function inspectEvolutionInboundPayload(payload: unknown): EvolutionInboundInspection {
  ...
  const hasInstanceId = Boolean(stringValue(payload.instanceId) || stringValue(payload.instance));
