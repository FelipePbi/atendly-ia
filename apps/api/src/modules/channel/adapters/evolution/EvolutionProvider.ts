...
export class EvolutionProvider implements WhatsAppProvider {
  ...
  async sendText(input: SendTextInput): Promise<SendTextResult> {
    ...
    const response = await fetch(url, {
      method: "POST",
      headers: buildHeaders(this.instanceTokenOverride),
...
function buildHeaders(instanceTokenOverride?: string): Record<string, string> {
  const headers: Record<string, string> = {
    "content-type": "application/json"
  };

  const sendApiKey = instanceTokenOverride || env.EVOLUTION_INSTANCE_TOKEN || env.EVOLUTION_API_KEY;
