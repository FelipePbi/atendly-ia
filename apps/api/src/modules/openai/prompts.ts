...
import { normalizeBusinessSettings, type BusinessSettingsDTO } from "../business-settings/business-settings.js";
import {
...
export function buildSystemPrompt(input: BuildSystemPromptInput | unknown): string {
  ...
  const virtualAttendantSettings = normalizeVirtualAttendantSettings(args.virtualAttendantSettings);
  ...
  return [
    ...
    JSON.stringify(
      {
        action: "send_message | call_tool | create_appointment | update_appointment_draft | pause_ai | handoff_human | do_nothing",
        ...
        pauseReason: "supplier_or_partner | personal_contact | human_requested | complaint_or_sensitive | spam | low_confidence | manual_handoff",
        conversationStage: "QUALIFYING_CONTACT",
        classification: "potential_customer | existing_customer | supplier_or_partner | personal_contact | spam | unknown",
        confidence: 0.9,
