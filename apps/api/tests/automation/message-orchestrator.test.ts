...
function buildSubject(options: { debounce?: false | { minSeconds: number; maxSeconds: number; maxWaitSeconds: number } } = {}) {
  ...
  return {
    ...
    idempotency,
    handoff,
    orchestrator: new MessageOrchestrator(automation, provider, idempotency, handoff, undefined, {
...
describe("MessageOrchestrator", () => {
  ...
  it("pauses indefinitely with /ia_pause when sent by the connected number", async () => {
    ...
    await expect(subject.orchestrator.handleInboundMessage(baseMessage({ messageId: "message-1", text: "Oi" }))).resolves.toMatchObject({
      action: "buffered"
    });

    await expect(
      subject.orchestrator.handleInboundMessage(baseMessage({ messageId: "command-1", fromMe: true, text: "/ia_pause" }))
    ).resolves.toMatchObject({
    ...
    await expect(subject.orchestrator.flushBufferedMessagesForTesting("5511999999999")).resolves.toBeNull();
  });

  it("does not pause when /ia_pause is sent by the customer", async () => {
    const subject = buildSubject();

    await expect(subject.orchestrator.handleInboundMessage(baseMessage({ text: "/ia_pause" }))).resolves.toMatchObject({
      action: "replied"
    });
  ...
  it("ignores fromMe messages that were sent by the bot", async () => {
    ...
    await expect(
      subject.orchestrator.handleInboundMessage(baseMessage({ fromMe: true, text: "resposta do bot" }))
    ).resolves.toMatchObject({
  ...
  it("resumes and responds when a text arrives after an unsupported-message pause", async () => {
    const subject = buildSubject();
    subject.handoff.isBotPaused.mockResolvedValueOnce(true).mockResolvedValue(false);
  ...
  it("sends assistant replies through the Evolution provider", async () => {
    ...
    const result = await subject.orchestrator.handleInboundMessage(baseMessage());
  ...
  it("checks for a pause again before sending the assistant reply", async () => {
    const subject = buildSubject();
    subject.handoff.isBotPaused.mockResolvedValueOnce(false).mockResolvedValueOnce(false).mockResolvedValueOnce(true);
  ...
  it("buffers consecutive customer messages and processes them as one grouped turn", async () => {
    ...
    await expect(
      subject.orchestrator.handleInboundMessage(baseMessage({ messageId: "message-2", text: "queria horario pra sobrancelha amanha" }))
    ).resolves.toMatchObject({
      action: "buffered"
    });
    ...
    await expect(subject.orchestrator.flushBufferedMessagesForTesting("5511999999999")).resolves.toMatchObject({
      action: "replied"
    });
