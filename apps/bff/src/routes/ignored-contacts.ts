...
import { dispatchToAiOrchestrator } from "../services/ai-orchestrator.js";
...
export async function registerIgnoredContactsRoutes(
  app: FastifyInstance,
): Promise<void> {
  ...
  app.post("/ignored-contacts", async (request, reply) => {
    ...
    if (!instance) {
      throw new AppError(
        "CONFLICT",
        "Connect WhatsApp before adding ignored contacts.",
        409,
      );
    }
    ...
    if (!jid) {
      throw new AppError(
        "VALIDATION_ERROR",
        "A valid jid or phoneNumber is required.",
        400,
      );
    }

    const reason =
      cleanOptionalText(data.reason) ?? "Contato adicionado manualmente";
  ...
  app.delete("/ignored-contacts/:id", async (request) => {
    ...
    const contact = await resumeIgnoredContactById({
      userId: user.id,
      id: params.id,
    });
...
async function bulkUpsertIgnoredContacts(input: {
  ...
  const uniqueContacts = [
    ...new Map(normalized.map((contact) => [contact.jid, contact])).values(),
  ];
  ...
  const reason =
    cleanOptionalText(input.reason) ?? "Selecionado nos contatos do WhatsApp";
...
async function resumeBotHandoffInApi(
  request: FastifyRequest,
  userId: string,
  jid: string,
): Promise<void> {
