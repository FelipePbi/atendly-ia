...
vi.mock("../src/services/evolution-go.js", () => ({
  buildWebhookUrl: () => "https://bff.example.com/webhooks/evolution-go?token=redacted",
...
beforeEach(() => {
  ...
  mocks.update.mockImplementation(async ({ data }: { data: Record<string, unknown> }) => ({ ...pendingInstance, ...data }));
});
...
describe("POST /whatsapp/pair", () => {
  it("isolates by authenticated user, normalizes the phone and reuses the pending instance", async () => {
    ...
    expect(mocks.connect).toHaveBeenCalledWith("instance-secret", expect.stringContaining("/webhooks/evolution-go"));
  ...
  it("returns the connected state without generating another code", async () => {
    mocks.status.mockResolvedValue({ connected: true, phoneNumber: "5511999999999" });
    ...
    expect(response.statusCode).toBe(200);
    expect(response.json().data).toMatchObject({ pairingCode: null, connected: true });
  ...
  it("returns a safe upstream failure without exposing the pairing code or token", async () => {
    mocks.pair.mockRejectedValue(new AppError("UPSTREAM_ERROR", "Evolution unavailable.", 502));
    const app = await pairingApp();
    const response = await app.inject({
      method: "POST",
      url: "/whatsapp/pair",
    ...
    expect(response.json()).toMatchObject({
      error: { code: "UPSTREAM_ERROR", message: "Não foi possível gerar o código de conexão." }
    });
    expect(response.body).not.toContain("instance-secret");
    await app.close();
  });
