...
export async function GET() {
  try {
    ...
    return ok({ ok: true, settings: settingsDto(settings) });
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (data.aiEnabled) {
      ...
      const phones = [
        ...new Set(conversations.flatMap((conversation) => whatsappPhoneCandidates(conversation.contactJid))),
      ];
    ...
    return ok({ ok: true, settings: settingsDto(settings), automationSync });
  } catch (error) {
    return handleRouteError(error);
  }
}
