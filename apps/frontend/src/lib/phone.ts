...
export function whatsappJidCandidates(value: string): string[] {
  const normalizedJid = normalizeWhatsappJid(value);
  if (!normalizedJid) return [];

  if (normalizedJid.endsWith("@g.us") || normalizedJid.endsWith("@lid")) {
    return [normalizedJid];
  }

  return whatsappPhoneCandidates(normalizedJid).map((phone) => `${phone}@s.whatsapp.net`);
}

export function whatsappConversationDedupeKey(value: string): string {
  const normalizedJid = normalizeWhatsappJid(value);
