...
A estrutura `apps/*`, o BFF base e os documentos de arquitetura alvo ja foram criados no monorepo. O frontend ja usa o BFF para autenticacao JWT, guards server-side, logout/login/register, onboarding, rotas WhatsApp principais, importacao de contatos, configuracoes de negocio, configuracoes da Atendente Virtual, lista de ignorados, rotas principais de conversas e ativacao/desativacao de IA. A proxima etapa e migrar as rotas restantes de webhook legado e persona customizada para remover as chamadas server-side diretas para API/Evolution Go.
