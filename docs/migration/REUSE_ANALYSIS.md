...
| Scheduling core | Transações, cálculo de disponibilidade, multi-serviço e snapshots; agenda refeita no Goal008 (política única de escrita em `write-policy.ts` para confirmar, remarcar, cancelar, bloquear e holds, resultado idempotente no mesmo commit do efeito, hold pelo relógio do banco, quatro estados do produto e histórico por evento) | REFACTOR (feito para os writers de agenda) | Unidade transacional preservada, sem novo serviço, banco ou fila. Restam exceções, compromisso pessoal, bloqueios recorrentes e séries (Goal009), importação (Goal010) e as telas (Goal016) |
