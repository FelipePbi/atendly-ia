...
- A estrutura atual preserva regras antigas de fonte externa operacional e onboarding que exige WhatsApp; essas regras não podem orientar o novo frontend. Os dois tons deixaram de ser fato do runtime no Goal011 — o estilo tem três valores do frontend ao prompt, com os antigos sobrevivendo só como alias de entrada até o Goal024.
...
| ProductOnboardingScreen + OnboardingRuntime | Fluxo legado fonte externa/WA obrigatório; a tela legada deixou de oferecer tom que o backend recusa no Goal011, mas continua sendo o fluxo antigo | REPLACE | Quatro blocos percebidos e progressivo; contratos de importação, mínimos operacionais, demo, estilo e ativação independentes |
...
| Scheduling core | Transações, cálculo de disponibilidade, multi-serviço e snapshots; agenda refeita no Goal008 (política única de escrita em `write-policy.ts` para confirmar, remarcar, cancelar, bloquear e holds, resultado idempotente no mesmo commit do efeito, hold pelo relógio do banco, quatro estados do produto e histórico por evento) e ocupação completada no Goal009 (regras de oferta do negócio aplicadas dentro do próprio motor de disponibilidade, buffer do catálogo como ocupação externa com snapshot, exceções geridas por rota e séries finitas de bloqueio e de atendimento sob a mesma política) | REFACTOR (feito para os writers e para o motor de disponibilidade) | Unidade transacional preservada, sem novo serviço, banco, fila ou fonte de agenda; o cálculo de intervalos existente foi reaproveitado, não reescrito. Restam as telas (Goal016/018); a importação foi entregue no Goal010, reusando `write-policy.ts`, a idempotência com resultado no mesmo commit e o padrão de lease pelo relógio do banco em vez de criar mecanismo paralelo |
| CalendarProvider como escolha operacional | Duas agendas | REMOVE (feito no Goal010 para escrita e oferta) | `CalendarProviderFactory` não importa mais `MinhaAgendaCalendarProvider` e recusa a fonte remota com `409 MINHA_AGENDA_OPERATIONAL_SOURCE_DISABLED`; o gateway da agenda local foi preservado e o adapter externo ficou exclusivo da importação, chamado direto pelo módulo. Resta remover o vocabulário legado (`EXTERNAL`, campos de `source`/`target`) no Goal024 |
| Minha Agenda leitor/cifra/conversão horários | Conhecimento real da integração | REFACTOR (feito no Goal010) | Reusado para importação sem reescrever a integração: a resposta passou a ser validada por schema antes de qualquer mapeamento, a leitura é por categoria com `raw` preservado e sem os filtros operacionais, e cobertura/limitação são declaradas por categoria em vez de presumidas. A cifra com AAD por tenant foi reusada como estava. O que a origem comprovadamente não fornece está declarado e quantificado; o resto continua sem prova contra a API real |
| Minha Agenda escrita/source switching | Incompatível com produto | REMOVE (corte feito no Goal010; código removido no Goal024) | Nenhum caminho operacional novo escreve na origem nem troca de fonte: `connect` recusa `enableWrites` com `409 INTEGRATION_WRITES_NOT_SUPPORTED` e desconectar a origem não desativa a agenda. O protocolo antigo (`importToAtendly`, migração bidirecional) sobrevive como compatibilidade declarada até o Goal024 e já não reabre o direito de importação (`assertImportAvailable`); sem fallback remoto |
...
| Cliente e serviço Scheduling | Cliente refeito no Goal006 (pessoa por ID, responsável, notas e tags com autorização, criação só na confirmação) e serviço refeito no Goal007 (quatro semânticas de preço, duração opcional com revisão, predicado operacional único, atributos do MVP, snapshot do acordo com total por regra única) | REFACTOR (feito) | Buffers na ocupação (Goal009), importação (Goal010), recorrência ofertável pela IA feita no Goal011 (`/internal/services` passou a emitir `recurrenceIntervalDays`), preferências e memória feitas no Goal012 (a memória da pessoa vive na IA em `CustomerMemory`, referenciando o `customerId` do Scheduling por ID, e o resumo só consome notas e tags `aiAuthorized` pela porta já existente), telas (Goal015) |
| IA graph/tools/LLM gateway | Separação entre modelo e efeito de agenda útil; políticas de conversa endurecidas no Goal011 e conhecimento/memória/sugestões no Goal012, sem trocar framework, provider nem fronteira | REFACTOR (feito para políticas, conhecimento e memória) | O grafo e as tools existentes foram reusados: a confirmação explícita virou regra de código (rascunho preparado em turno anterior, com códigos de recusa próprios), o erro de infraestrutura deixou de entrar no contexto do modelo, o hold do rascunho substituído passou a ser liberado no mesmo caminho do estado e o prompt ficou sem persona com versão derivada do conteúdo. Testes de falha, confirmação e handoff existem como evals determinísticos com modelo dublê no `validate:core`. O Goal012 reusou a mesma estrutura para conhecimento, memória e sugestões sem trocar framework nem fronteira: a recuperação continua sendo o nó `retrieval` do grafo, agora filtrando por serviço em foco e apresentando a precedência do produto; a memória da pessoa entrou como módulo próprio ao lado da `ConversationMemory` de sessão, sem substituí-la; e a sugestão ao humano roda **fora** do grafo, com binding somente leitura das tools existentes, para que nenhuma tool com efeito seja sequer oferecida ao modelo. O Goal013 fechou mídia sobre a mesma estrutura, também sem trocar framework, provider de modelo nem fronteira: a transcrição entrou como porta própria (`TranscriptionProvider`, implementação OpenAI por HTTP na conta já configurada, dublê em teste) atrás de um nó novo do grafo entre `recordInbound` e `sessionGate`, e imagem reusa o nó `handoff` que já existia, de forma determinística e sem chamar o modelo. Nada de mídia sobrou para depois exceto exibição (Goal017) e retenção (Goal022) |
...
| Conversas/Contact/sessão | Entregue no Goal005: `Contact` e `ConversationSession` persistidos, categoria com override manual, ignore absoluto e controle humano determinístico fora do checkpoint do modelo | REFACTOR / CREATE (feito) | Classificação do agente rebaixada a sugestão com proveniência; restam identidade de pessoa e memória (Goals 006 e 012) e a inbox em três abas (Goal017) |
| RAG/pgvector/checkpointer | Filtros e persistência úteis | REFACTOR (feito para o CRUD versionado no Goal012) | O store pgvector foi mantido — nenhuma troca de vector DB, de provider de embedding nem de dimensão. O Goal012 acrescentou o serviço de documentos por cima dele: versão nova com inativação da anterior na mesma transação, embedding antes da transação (`KNOWLEDGE_INDEX_UNAVAILABLE` sem tocar o banco), `checksum` igual sem reindexar e filtro por serviço em foco na busca, com o seed rebaixado a ferramenta de desenvolvimento. Consentimento e expiração de cópias continuam abertos (retenção é do Goal022) |
...
| Jobs de domínio | Primeiro loop entregue no Goal008: conclusão automática dentro do próprio Scheduling, com lease por `pg_try_advisory_xact_lock`, intervalo configurável, desligamento por variável e disparo por rota interna | CREATE (primeiro caso feito) | Confirma D-008 sem fila externa nem novo serviço; o Health Worker continua só sondando. Lembretes, purge e importação ainda precisam do mecanismo comum (Goals 010 e 021) |
