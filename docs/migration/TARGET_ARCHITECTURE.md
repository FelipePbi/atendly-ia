...
Serviço mantém quatro tipos: fixo, a partir de, sob consulta, não informado. Valores monetários preservam Decimal/semântica consistente no contrato; não usar float como autoridade nem zero para desconhecido. Serviço importado sem duração fica pendente, não operacional. Snapshot do atendimento mantém o acordo e versão/proveniência; edição do catálogo não o reescreve. Status do atendimento, presença pelo lembrete e valor final cobrado são dados separados. Goal007 implementa a parte de catálogo e snapshot: `PriceType` com quatro valores e constraints SQL, duração opcional com revisão em lockstep, predicado operacional único para qualquer fonte, atributos do MVP persistidos, `computeAgreementTotal` com `totalPriceType` e mapper de importação sem semântica fabricada (D-023); versão/proveniência da proposta, presença e valor final seguem para o Goal008.
