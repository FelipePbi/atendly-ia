...
Customer possui identidade da pessoa e contatos informados; Contact da IA representa o endereço de transporte. Um Contact pode corresponder a várias pessoas. Relação `(tenantId, contactId, customerId)` não autoriza decidir a pessoa pelo telefone. Scheduling é autoritativo para identidade do cliente; IA guarda referência/mapeamento e confirma ambiguidades na operação. Goal006 implementa esta camada: `Customer` por ID com telefone opcional e não exclusivo, candidatos por número com seleção explícita, criação só dentro da transação de confirmação, `Contact.customerId` por referência sem FK entre bancos, responsável principal confirmado e notas/tags com `aiAuthorized` (D-022).

Configuração comercial privada e notas/tags com autorização explícita só são fornecidas às ferramentas permitidas. Não copiar todas as notas para embeddings. Memória inferida carrega origem, cliente, timestamps/validade e permissão; a alteração de Ignorar IA invalida contexto/checkpoints pertinentes antes do próximo processamento. Goal012 implementa esta parte em `CustomerMemory` (D-028): permissão é coluna **por linha**, negada por padrão e filtrada na própria consulta; a inferência exige efeito verificável no ledger de tools do turno, nunca o texto da decisão; contradição substitui sem apagar; e o resumo do cliente só vê material autorizado e nunca roda sem `AiRun`.
...
- APIs internas exigem credencial de serviço, contexto validado e escopo da operação. Adotar credenciais distintas por chamador/uso (provisionamento, comandos, leitura, transporte), com configuração de permissões no receptor; `x-service-audience` sozinho não autentica o chamador. Sem novo identity provider. Goal003 implementa a derivação por chamador/audiência/uso e a autorização por escopo no receptor (D-019).
...
- BFF mantém gestão administrativa da credencial; IA usa credencial cifrada/projeção controlada ou resolução interna do vínculo, nunca token livre enviado no corpo do webhook como autoridade. Rotação/reconciliação registra versão; falha não deve silenciosamente voltar a chave global. Goal003 implementa a cifra no BFF e a projeção cifrada na IA; a retirada do token do payload do produtor Go fica para o Goal004.
- Cookies de sessão com CSRF/origem validados, revogação de sessão/lifecycle e configuração segura por ambiente. CORS isoladamente não é controle completo de CSRF. Goal003 implementa sessão revogável e CSRF por origem mais token vinculado à sessão, entregue também por header de resposta exposto à origem permitida, porque frontend e BFF vivem em hosts distintos. A recomendação é compatível com a [orientação OWASP](https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html), consultada em 2026-09-05; exploração do runtime não foi realizada.
...
Preservar Serializable/advisory locks existentes, estendendo protocolo a bloqueios, exceções, regras, holds e remarcação, com ordem estável de locks e retry limitado de abortos serializáveis. Não impor constraint de não sobreposição que elimine exceções humanas explicitamente permitidas pelo produto. IA jamais aplica override; comando humano registra confirmação consciente, ator e motivo quando aplicável. Goal008 implementa esta política em `calendar/write-policy.ts`, único caminho de escrita da agenda: transação Serializable, lock de todos os dias afetados em ordem estável de datas, revalidação dentro da transação e retry limitado de `40001`/`40P01`/`P2034` com erro próprio no limite; cancelamento e bloqueio deixaram de escrever fora dela; o override humano exige `source: USER`, flag e motivo, e fica registrado no evento (D-006, D-024). Exceções, regras e séries entram sob a mesma política no Goal009.

Hold dura cinco minutos conforme vault; validade é checada pelo relógio do banco no comando, sem depender de worker pontual. Remarcação mantém original enquanto nova opção não é confirmada; commit altera compromisso/ocorrência e histórico. Cancelar preserva história e invalida jobs ligados à versão anterior. Goal008 entrega o hold como `AppointmentHold`, com `expiresAt` em TIMESTAMPTZ calculado pelo `now()` do banco e vigência sempre reavaliada contra o relógio do banco — hold vencido é ignorado sem worker, e hold vigente ocupa o horário para todos exceto para a confirmação que o consome; a remarcação com hold mantém o original reservado até o commit, que atualiza o atendimento, libera o hold e grava o evento com antes/depois. Recorrência possui série e ocorrências finitas, exceções e edição por ocorrência/série conforme fluxo de produto; sem gerar série infinita silenciosa. Goal009 entrega essa parte (D-025): séries de bloqueio/compromisso e de atendimento são finitas por data ou por contagem, com teto configurável e ocorrências **materializadas** na criação — nunca geradas sob demanda, justamente para que exceção por ocorrência, conflito por ocorrência e lock por dia existam; a série de atendimento tem um hold por ocorrência e confirmação global em uma transação, e depois de confirmada as ocorrências são atendimentos independentes ligados só por referência de leitura. A oferta de horário deixa de ser decidida por quem chama: antecedência mínima, antecedência máxima e granularidade são dado persistido do tenant, aplicados dentro do motor contra o relógio do banco, e o buffer do serviço passa a ocupar tempo como intervalo externo ao atendimento, gravado como snapshot na confirmação para que editar o catálogo não mova ocupação já confirmada.

Serviço mantém quatro tipos: fixo, a partir de, sob consulta, não informado. Valores monetários preservam Decimal/semântica consistente no contrato; não usar float como autoridade nem zero para desconhecido. Serviço importado sem duração fica pendente, não operacional. Snapshot do atendimento mantém o acordo e versão/proveniência; edição do catálogo não o reescreve. Status do atendimento, presença pelo lembrete e valor final cobrado são dados separados. Goal008 implementa esta parte: quatro estados do produto com constraint SQL e bruto anterior preservado em `statusRaw`, presença e valor final como colunas próprias (valor final nunca inferido do preço previsto), histórico append-only em `AppointmentEvent` gravado na mesma transação de cada mutação, e conclusão automática trinta minutos após o término por loop com lease dentro do próprio Scheduling (D-008); a presença é alimentada pelos lembretes no Goal021. Goal007 implementa a parte de catálogo e snapshot: `PriceType` com quatro valores e constraints SQL, duração opcional com revisão em lockstep, predicado operacional único para qualquer fonte, atributos do MVP persistidos, `computeAgreementTotal` com `totalPriceType` e mapper de importação sem semântica fabricada (D-023); versão/proveniência da proposta, presença e valor final seguem para o Goal008.
...
Tools de agenda retornam fatos, IDs, versão/resultados e erros de domínio; LLM não acessa DB de agenda. Confirmação final do cliente precisa apontar proposta conhecida e ainda válida. Falha de consulta não usa agenda antiga como verdade; erro crítico vai para humano sem mensagem automática de infraestrutura. Goal011 implementa esta camada sem mudar a fronteira: o erro de domínio chega ao modelo com código próprio e o de infraestrutura vira um único código opaco cujo detalhe existe só no log, com `requestId`/`aiRunId`; tool com efeito exige rascunho preparado em turno anterior, e propor outro horário ou descartar a intenção libera o hold anterior no mesmo caminho que grava o estado; nenhuma tool alcança encaixe, override, exceção ou desconto, o que é asserção executável e não convenção (D-027). Prompts/RAG são dados não confiáveis para autorização e respeitam filtros de privacidade fora do modelo.

Áudio: obter attachment sob autorização, transcrever, guardar proveniência e tratar confirmação clara com mesmas invariantes. Imagem: mostrar e encaminhar a humano; documento: mostrar sem interpretação. Não há ingestão de PDF nem visão no MVP. Embeddings somente do conhecimento/projeções permitidas; versão de documento e índice devem ser reconciliáveis. Criar FAQ/conhecimento/sugestões pelo módulo, não por seed como interface operacional. Goal012 entrega esta camada (D-028): conhecimento é criado, versionado e desativado por rota, com versão nova e inativação da anterior na mesma transação e com o embedding calculado **antes** dela, de modo que documento ativo sem chunks é impossível; a recuperação filtra tenant, `ACTIVE` e serviço em foco, e o prompt apresenta a precedência do produto — regra do serviço em foco > FAQ > dados estruturados > campo livre; a sugestão ao humano roda fora do grafo, com binding somente leitura, e nunca autoenvia. Goal013 entrega a parte de mídia (D-029): toda mensagem do contato existe na inbox com `kind` e mídia tem `MessageAttachment` de metadados, sem byte retido pela IA — o `base64` do provedor é purgado ao concluir o evento, com backfill do estoque, e os bytes necessários depois disso vêm da origem pelas chaves do proto; a transcrição é uma porta com dublê em teste, roda entre `recordInbound` e `sessionGate` — inclusive com a IA desligada e em atendimento humano, nunca para contato ignorado ou sessão pessoal —, vira o turno marcada como transcrição e nunca inventa texto quando falha; imagem gera handoff determinístico sem chamar o modelo, documento e vídeo aparecem sem interpretação, e sticker, GIF e desconhecido não decidem nada. Continua sem visão e sem ingestão de PDF, e sem armazenamento durável próprio de mídia.
...
Goal010 entrega esta parte (D-026). A sessão é `ImportSession`, com `ImportSessionCategory`, `ImportItem` e `ImportDecision`, e o **direito de importação deixou de ser regra de código**: ele mora em `ImportSession.completedAt` e é garantido pelo índice único **parcial** `ImportSession_one_completed_per_tenant`, de modo que a segunda conclusão é recusada pelo banco mesmo com duas conexões concorrentes; `ImportSession_one_live_per_tenant` faz a "uma sessão por negócio", e substituir uma sessão viva exige movê-la antes para `SUPERSEDED`/`FAILED` — falha técnica antes da conclusão não consome o direito. A análise não escreve dado operacional e é versionada: cada reanálise incrementa `previewVersion` e reconcilia por `(category, externalId)` sem apagar decisão nem reverter item já executado, e executar exige a versão aprovada (preview obsoleto é recusado, nunca seguido em silêncio). A execução é por item sob a política única de escrita, com efeito, mapa de origem e checkpoint no mesmo commit, coordenada por lease pelo relógio do banco na própria linha da sessão — não por agendamento em memória, que nunca foi coordenação entre instâncias. A importação não sobrepõe agenda: horário futuro já ocupado por atendimento que não veio dela devolve a decisão ao negócio em vez de escrever por cima. O adapter permanece read-only e declara por categoria o que a origem não fornece, em vez de fabricar dado; a importação é só humano, sem contrato do lado da IA.

Migração interna de bancos não consome esse direito. Negócios que operam no legado precisam inventário e janela de corte: congelar escritor remoto, drenar comandos, obter/reconciliar snapshot e ativar único escritor local. Goal010 corta o escritor remoto no código — a factory recusa a fonte remota para escrita e oferta, e a leitura sobrevive só dentro da importação — e reconcilia os jobs legados por **classificação**, nunca conversão: lote técnico concluído não é conclusão de importação do usuário e não consome o direito. A janela de corte por tenant, contra a origem real, continua sendo operação, não código. Durante essa janela, informar indisponibilidade/pendência real; não oferecer duas agendas operacionais. Código antigo pode permanecer instalado para reconciliação, mas não como alternativa selecionável do produto novo. [DATA_MIGRATION](DATA_MIGRATION.md) define ensaio/rollback.
