...
import { rehearsalTarget } from "../goal013-media-migration-rehearsal.mjs";

const validUrl = "postgresql://pgtest:pgtest@127.0.0.1:55432/atendly_bff_test";

test("derives a disposable database dedicated to the Goal013 rehearsal", () => {
  const target = rehearsalTarget({ BFF_TEST_DATABASE_URL: validUrl });

  assert.equal(target.name, "atendly_bff_test_goal013");
  assert.equal(target.label, "127.0.0.1:55432/atendly_bff_test_goal013");
...
test("refuses to run without an explicit test database URL", () => {
  const outcome = spawnSync(
    process.execPath,
    [path.join(repositoryRoot, "scripts", "goal013-media-migration-rehearsal.mjs")],
